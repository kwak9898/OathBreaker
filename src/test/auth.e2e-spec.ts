...
describe("회원 인증 관련 테스트", () => {
  ...
  describe("로그인 테스트", () => {
    ...
    it("비밀번호가 틀렸을 때 실패", async () => {
      ...
      expect(body.code).toBe(AUTH_EXCEPTION.AUTH_FAIL_VALIDATE.code);
      expect(body.message).toBe(AUTH_EXCEPTION.AUTH_FAIL_VALIDATE.message);
    });

    it("아이디가 틀렸을 때 실패", async () => {
      // Given
